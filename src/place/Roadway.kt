...
class Roadway(
    override val previous: Node,
    override val next: Node,
    override val meters: Int,
    val busFreq: Int,
    val busCost: Int
) :
    Way {
    ...
    override val cellMaxAgents = 2 // 50m間隔（広い？）
    ...
    override fun checkAllAgents(time: Int) {
        for (i in 0 until cellNum) {
            ...
            if (index == cellNum - 1) { // 最後のマス
                ...
                for (vehicle in removingVehicles) {
                    vehicles[index].remove(vehicle)
                } // 車を消滅させる
            } else {
                val removingVehicles = ArrayList<Vehicle>()
                for (vehicle in vehicles[index]) {
